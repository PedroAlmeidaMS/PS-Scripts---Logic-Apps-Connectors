...
#Get Resource Group Info to be used in Az REST command
...
$resources | ForEach-Object {    
    if($_.Kind -eq 'V1')
    {
        Write-Host 'Connector: ' ($_.Properties.api.psobject.Properties | Where-Object {$_.Name -eq "displayName"}).Value  ' | Connector Name: '$_.Name ' | Status:' $_.Properties.overallStatus '| connected user:' $_.Properties.authenticatedUser.psobject.Properties.value 
...
$connectorDictionary.Values | ForEach-Object{
    ...
    if($_.IsUsed -eq 'FALSE')
    {
        Write-Host $_.name ': is an orphan | ID:' $_.Id
    }
}
